...
const clientesConLogo = [
  ['Renacer', 'https://logo.clearbit.com/bodegarenacer.com.ar'],
  ['Luigi Bosca', 'https://logo.clearbit.com/luigibosca.com'],
  ['Famiq', 'https://logo.clearbit.com/famiq.com.ar'],
  ['Salentein', 'https://logo.clearbit.com/bodegasalentein.com'],
  ['Bodega Norton', 'https://logo.clearbit.com/norton.com.ar'],
  ['Talca', 'https://logo.clearbit.com/talca.com.ar'],
  ['Randstad', 'https://logo.clearbit.com/randstad.com'],
  ['Servicio Meteorológico Nacional', 'https://logo.clearbit.com/smn.gob.ar'],
  ['Yaguar', 'https://logo.clearbit.com/yaguar.com.ar'],
  ['Rosell Boher', 'https://logo.clearbit.com/rosellboher.com'],
  ['Trivento', 'https://logo.clearbit.com/trivento.com'],
  ['Chandon', 'https://logo.clearbit.com/chandon.com.ar'],
  ['Park Hyatt', 'https://logo.clearbit.com/hyatt.com'],
...
const clientesTexto = ['Valle de Uco', 'Neverland'];
...
export default function App() {
  ...
  return (
    <main className="min-h-screen bg-white text-slate-900 scroll-smooth selection:bg-blue-100">
      ...
      <section id="clientes" className="bg-white py-24 sm:py-32"><div className="mx-auto max-w-7xl px-6 lg:px-10"><div className="mb-14 text-center"><Kicker>Nuestros clientes</Kicker><h2 className="text-4xl font-black uppercase leading-none tracking-[-0.05em] text-[#1a365d] sm:text-5xl">Empresas que confían</h2></div><div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">{clientesConLogo.map(([name, src]) => <div key={name} className="group flex h-28 items-center justify-center border border-slate-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:shadow-xl"><img src={src} alt={name} className="max-h-16 max-w-full object-contain grayscale transition-all group-hover:grayscale-0" loading="lazy" referrerPolicy="no-referrer" onError={(e) => { const img = e.currentTarget; img.style.display = 'none'; const next = img.nextElementSibling as HTMLElement | null; if (next) next.style.display = 'block'; }} /><span className="hidden text-center text-xs font-black uppercase tracking-widest text-slate-500">{name}</span></div>)}{clientesTexto.map(c => <div key={c} className="flex h-28 items-center justify-center border bg-white p-4 text-center text-xs font-black uppercase tracking-widest text-slate-500 shadow-sm">{c}</div>)}</div></div></section>
