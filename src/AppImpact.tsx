...
const clientes = [
  ...
  ['Randstad', 'Servicios'],
  ['SMN', 'Organismo'],
  ['Halliburton', 'Industria'],
  ['Coca-Cola', 'Consumo masivo'],
      ...
      <section id="clientes" className="bg-white py-24 sm:py-32">
        <div className="mx-auto max-w-7xl px-6 lg:px-10">
          <div className="mb-14 text-center">
            ...
            <h2 className="text-4xl font-black uppercase leading-none tracking-[-0.05em] text-[#1a365d] sm:text-5xl">Empresas que confían</h2>
            <p className="mx-auto mt-6 max-w-3xl text-lg font-light leading-relaxed text-slate-500">Una cartera diversa de empresas, bodegas, industrias, hoteles y organizaciones que requieren respuesta técnica, continuidad y respaldo profesional.</p>
          </div>

          <div className="mb-6 grid gap-4 md:grid-cols-4">
            {clientesTop.map(([nombre, rubro]) => (
              <div key={nombre} className="relative overflow-hidden bg-[#0a192f] p-7 text-white shadow-xl">
                <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#1a365d] to-[#4299e1]" />
                <p className="text-[9px] font-black uppercase tracking-[0.35em] text-white/40">{rubro}</p>
                <p className="mt-8 min-h-[64px] text-2xl font-black uppercase leading-none tracking-[-0.04em]">{nombre}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-7">
            {clientes.map(([nombre, rubro]) => (
              <div key={nombre} className="group flex min-h-28 flex-col justify-between border border-slate-100 bg-white p-5 shadow-sm transition-all hover:-translate-y-1 hover:border-[#3b82f6] hover:shadow-xl">
                <p className="text-[8px] font-black uppercase tracking-[0.28em] text-[#3b82f6]">{rubro}</p>
                <p className="mt-8 text-base font-black uppercase leading-tight tracking-[-0.03em] text-[#1a365d] group-hover:text-slate-950">{nombre}</p>
              </div>
            ))}
          </div>
        </div>
